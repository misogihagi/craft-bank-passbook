...
import { Clock, FileText, MoreHorizontal, Printer, Search, X } from "lucide-react"
...
const mockJobs: PrintJob[] = [
  {
    id: "1",
    user: { name: "Sarah Chen", email: "sarah.chen@company.com", avatar: "/placeholder.svg?height=32&width=32" },
  ...
  {
    id: "3",
    user: { name: "Emily Rodriguez", email: "e.rodriguez@company.com", avatar: "/placeholder.svg?height=32&width=32" },
  ...
  {
    id: "5",
    user: { name: "Lisa Wang", email: "lisa.wang@company.com", avatar: "/placeholder.svg?height=32&width=32" },
...
export default function Component() {
  ...
  const handlePauseJob = (jobId: string) => {
    setJobs(jobs.map((job) => (job.id === jobId ? { ...job, status: "paused" as const } : job)))
  }

  const handleResumeJob = (jobId: string) => {
    setJobs(jobs.map((job) => (job.id === jobId ? { ...job, status: "queued" as const } : job)))
  }
    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Monitor and manage print jobs across all printers</p>
        </div>
        <div className="flex items-center gap-2">
          <Printer className="w-5 h-5 text-muted-foreground" />
          <span className="text-sm text-muted-foreground">HP LaserJet Pro 4000</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">Currently Printing</CardTitle>
            <Printer className="h-4 w-4 text-blue-600" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold text-blue-600">{queueStats.printing}</div>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-orange-600">{queueStats.paused}</div>
          </CardContent>
        </Card>
      </div>
                ...
                <TableRow key={job.id}>
                  <TableCell>
                    <div className="flex items-center gap-3">
                      <Avatar className="w-8 h-8">
                        <AvatarImage src={job.user.avatar || "/placeholder.svg"} alt={job.user.name} />
                        <AvatarFallback>
                          {job.user.name
                            .split(" ")
                            .map((n) => n[0])
                  ...
                  <TableCell>
                    <div className="text-sm">
                      ...
                      <div className="text-muted-foreground">{getTimeAgo(job.submittedAt)}</div>
                    </div>
                  </TableCell>
                  <TableCell>
                    <div className="text-sm">
                      ...
                      <div className="text-muted-foreground">
                        {job.status === "printing" ? "In progress" : "Estimated"}
                      </div>
                    </div>
                  </TableCell>
                  <TableCell>
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end">
                        {job.status === "paused" ? (
                          <DropdownMenuItem onClick={() => handleResumeJob(job.id)}>Resume Job</DropdownMenuItem>
                        ) : job.status === "queued" ? (
                          <DropdownMenuItem onClick={() => handlePauseJob(job.id)}>Pause Job</DropdownMenuItem>
                        ) : null}
                        <DropdownMenuItem>Move to Top</DropdownMenuItem>
                        <DropdownMenuItem>View Details</DropdownMenuItem>
                        <DropdownMenuItem className="text-red-600" onClick={() => handleCancelJob(job.id)}>
                          Cancel Job
                        </DropdownMenuItem>
                      </DropdownMenuContent>
                    </DropdownMenu>
